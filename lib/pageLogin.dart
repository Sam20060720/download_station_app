                        ...
                        try {
                          ...
                          try {
                            ...
                            if (decodedJSON['success'] == true) {
                              ...
                              tryLogin(context, 0, url.toString().contains('https') ? 'https://${Uri.parse(url.toString()).host}' : 'http://${Uri.parse(url.toString()).host}', _accountController.text,
                                  _passwordController.text, true, orginput);
                              setState(() {
                                ...
                                child: OutlinedButton(
                                  onPressed: () async {
                                    ...
                                    if (_adressController.text.contains('.') == false) {
                                      //當網址不包含.時 (使用 QuickConnect ID 登入)
                                      _adressController.text =
                                          'https://${_adressController.text}.quickconnect.to/webapi/auth.cgi?api=SYNO.API.Auth&version=3&method=login&account=${_accountController.text}&passwd=${_passwordController.text}&session=DownloadStation&format=sid';
                                      FAS.write(key: 'isquickconnect', value: 'true');
                                      setState(() {
                                        _useQuickConnect = true;
                                      });
                                    } else if (_adressController.text.contains('.quickconnect.to')) {
                                      //當網址包含.quickconnect.to時 (使用 QuickConnect ID 登入)
                                      var tmpurl = Uri.parse(_adressController.text).host;
                                      await FAS.write(key: 'isquickconnect', value: 'true');
                                      _adressController.text =
                                          'https://${_adressController.text}/webapi/auth.cgi?api=SYNO.API.Auth&version=3&method=login&account=${_accountController.text}&passwd=${_passwordController.text}&session=DownloadStation&format=sid';
                                      setState(() {
                                        _useQuickConnect = true;
                                      });
                                    } else {
